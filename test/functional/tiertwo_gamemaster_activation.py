...
"""
Test checking:
 1) Gamemaster setup/creation.
 2) Tier two network sync (gamemaster broadcasting).
 3) Gamemaster activation.
 4) Gamemaster expiration.
 5) Gamemaster re activation.
 6) Gamemaster removal.
 7) Gamemaster collateral spent removal.
"""
...
class GamemasterActivationTest(HemisTier2TestFramework):
    ...
    def reconnect_and_restart_gamemasters(self):
    ...
    # Similar to base class wait_until_gm_status but skipping the disconnected nodes
    def wait_until_gm_expired(self, _timeout, removed=False):
        collaterals = {
            self.remoteOnePos: self.gmOneCollateral.hash,
            self.remoteTwoPos: self.gmTwoCollateral.hash
        }
        for k in collaterals:
            for i in range(self.num_nodes):
                ...
                try:
                    if removed:
                        wait_until(lambda: (self.get_gm_status(self.nodes[i], collaterals[k]) == "" or
                                            self.get_gm_status(self.nodes[i], collaterals[k]) == "REMOVE"),
                                   timeout=_timeout, mocktime=self.advance_mocktime)
                    else:
                        wait_until(lambda: self.get_gm_status(self.nodes[i], collaterals[k]) == "EXPIRED",
                                   timeout=_timeout, mocktime=self.advance_mocktime)
                except AssertionError:
                    s = "EXPIRED" if not removed else "REMOVE"
                    strErr = "Unable to get status \"%s\" on node %d for gmode %s" % (s, i, collaterals[k])
    ...
    def run_test(self):
        ...
        self.wait_until_gm_expired(30)
        self.log.info("gamemasters expired successfully")

        # check gamemaster removal
        ...
        self.wait_until_gm_expired(30, removed=True)
        self.log.info("gamemasters removed successfully")

        # restart and check spending the collateral now.
        self.reconnect_and_restart_gamemasters()
        ...
        self.sync_blocks()
        self.log.info("checking gm status..")
        time.sleep(3)           # wait a little bit
        self.wait_until_gm_vinspent(self.gmOneCollateral.hash, 30, [self.remoteTwo])
        self.log.info("gamemaster list updated successfully, vin spent")




if __name__ == '__main__':
    GamemasterActivationTest().main()
