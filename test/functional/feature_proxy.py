...
"""Test hemisd with different proxy configuration.

Test plan:
- Start hemisd's with different proxy configurations
- Use addnode to initiate connections
- Verify that proxies are connected to, and the right connection command is given
- Proxy configurations to test on hemisd side:
...
- Create hemisds that connect to them
- Manipulate the hemisds using addnode (onetry) an observe effects
...
class ProxyTest(HemisTestFramework):
    ...
    def node_test(self, node, proxies, auth, test_onion=True):
        ...
        # Note: hemisd's SOCKS5 implementation only sends atyp DOMAINNAME, even if connecting directly to IPv4/IPv6
