...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("fShowGamemastersTab"))
        settings.setValue("fShowGamemastersTab", gamemasterConfig.getCount());
...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::HMS);
    ...
    if (settings.contains("nAnonymizeHemisAmount") || reset)
        gArgs.SoftSetArg("-anonymizeHemisamount", settings.value("nAnonymizeHemisAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in Hemis.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case ShowGamemastersTab:
            return settings.value("fShowGamemastersTab");
#endif
        case DisplayUnit:
            return nDisplayUnit;
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case SpendZeroConfChange:
            ...
            break;
        case ShowGamemastersTab:
            if (settings.value("fShowGamemastersTab") != value) {
                settings.setValue("fShowGamemastersTab", value);
                setRestartRequired(true);
            }
            break;
#endif
